...
Case of 
	: (Form event code:C388=On Load:K2:1)
		$template_obj:=New object:C1471("templateName";"standard pict button")
	...
	: (Form event code:C388=On Clicked:K2:4)
		...
		$nameTemplate:=Form:C1466.tips[Form:C1466.currentNameTemplate].TipName()+"_copy"
		$templatePath:=getTlocPath (Form:C1466.currentNameTemplate+$icon)
		
		
		  //save duplicate
		$folder:=Folder:C1567($templatePath;fk platform path:K87:2)
		
		If ($folder.name=Form:C1466.currentNameTemplate)
			$folderPref:=Folder:C1567(getTlocPath ($icon);fk platform path:K87:2)
			If ($folder.exists)
				$folder:=$folder.copyTo($folderPref;$nameTemplate)
				$templatePath:=Replace string:C233($templatePath;Form:C1466.currentNameTemplate;$nameTemplate)
			Else 
		...
		If (Not:C34($folderNotFound))
			$file:=File:C1566($templatePath+Form:C1466.currentNameTemplate+".json";fk platform path:K87:2)
			If ($file.exists)
				If ($file_copy)
				...
				End if 
				Form:C1466.tips[$nameTemplate]:=OB Copy:C1225(Form:C1466.tips[Form:C1466.currentNameTemplate])
				Form:C1466.tips[$nameTemplate].TipName($nameTemplate)
				Form:C1466.currentNameTemplate:=$nameTemplate
				Form:C1466.tips[Form:C1466.currentNameTemplate].Export($nameTemplate;$templatePath)
		...
		Form:C1466.tips[Form:C1466.currentNameTemplate].Show()
